...
<table>
	...
	<tr>
		<td colspan="2">
			<c:if test="${!empty transaction.description}">
				<input type="submit"
					value="<spring:message code="global.edit"/>" />
			</c:if>
			<c:if test="${empty transaction.description}">
				<input type="submit"
...
<c:if test="${!empty listTransactions}">
	...
	<c:forEach items="${listTransactions}" var="transaction">
		<tr>
			...
			<td><a href="<c:url value='/edit/${transaction.id}' />" ><spring:message code="global.edit" /></a></td>
			<td><a href="<c:url value='/remove/${transaction.id}' />" ><spring:message code="global.delete" /></a></td>
		</tr>
	</c:forEach>
	</table>
</c:if>
